...
if (empty($id) || !ctype_digit($id)) {
    header('Location: ../../../pages/edit_inaktif.php?id=' . urlencode($id) . '&status=invalid_id');
    exit;
}

if ($_SERVER['REQUEST_METHOD'] !== 'POST' || empty($_FILES['files'])) {
    header('Location: ../../../pages/edit_inaktif.php?id=' . urlencode($id) . '&status=no_files');
    exit;
}
...
header('Location: ../../../pages/edit_inaktif.php?id=' . urlencode($id) . '&status=uploaded');
exit;
?>